...
st.title("📚 CiteWise")
st.caption(
    "AI Citation Management Agent — detect claims, find supporting academic "
    "sources, and generate citations."
)
...
with st.sidebar:
    ...
    st.markdown("---")
    st.info(
        "Demo mode works with the included dataset. For live academic search, "
        "add an API key in `.env`."
    )
...
if result:
    ...
    with tab2:
        for item in result["claims"]:
            ...
            if item["needs_citation"]:
                ...
                st.info(
                    "Likely self-contained statement; citation not automatically "
                    "required."
                )
    ...
    st.info(
        "Upload the included demo paper or your own PDF/TXT file, "
        "then click Analyze Paper."
    )
